...
<template>
  <div>
    <div>
      The conclusion for
      <span class="underline">{{ getName(props.requesedAtomForExplanation) }}</span>
      ...
      <template v-else-if="hasOneEntry(significantAtomIds)">
        depends on <span class="underline">{{ getName(significantAtomIds[0]) }}</span
        >.
      </template>
      <template v-else-if="significantAtomIds.length > 1">
        ...
        <ul>
          <template v-for="atomId in significantAtomIds.slice(0, -1)" :key="`${atomId}`">
            <li>
              <span
                ><span class="underline">{{ getName(atomId) }}</span></span
              >
            </li>
          </template>
          <li>
